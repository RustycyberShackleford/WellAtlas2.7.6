...
def seed_demo_if_empty():
    ...
    deleted_sites = set(random.sample(range(50), 2))  # seed 2 as deleted
    job_num = 25001
    for i in range(50):
...
@app.route("/")
def index():
    return render_template("index.html", header_title="WellAtlas by Henry Suden")
...
def deleted_sites():
    ...
    return render_template("deleted_sites.html",
                           header_title="WellAtlas by Henry Suden",
                           sites=rows)
...
def site_detail(site_id):
    ...
    return render_template("site_detail.html",
                           header_title="WellAtlas by Henry Suden",
                           site=site, jobs=jobs, notes=notes, photos=photos)
...
def job_detail(site_id, job_id):
    ...
    return render_template("job_detail.html",
                           header_title="WellAtlas by Henry Suden",
                           site=site, job=job, jnotes=jnotes, jphotos=jphotos)
...
def upload_job_photo(site_id, job_id):
    ...
    if not file or file.filename == "":
        return redirect(url_for("job_detail", site_id=site_id, job_id=job_id))
    # allow any ext for demo; production would validate
    fname = file.filename
    base, ext = os.path.splitext(fname)
    uniq = datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
    fname = f"{base}_{uniq}{ext}"
